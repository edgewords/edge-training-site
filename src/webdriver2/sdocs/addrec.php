...
// add the record
function add_record ($name, $uname, $pn, $pw) {
	global $sql;
	$query = "INSERT INTO records (name, uname, pin, pw) VALUES ('$name', '$uname', '$pn', PASSWORD('$pw'))";
	$sql->Insert($query);
}
	...
	if (record_present($uname)) {
		$pageText .= "<h1>Found $uname exists in Database</h1>\n";
	}
	else {
		add_record($name, $uname, $pn, $pw);
		
		$pageText.= "<h1>$uname Record Added to Database</h1>\n";
	}
	echo $pageText;
