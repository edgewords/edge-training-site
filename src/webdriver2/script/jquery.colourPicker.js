...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		var icon	= jQuery('<a href="#"><img style="vertical-align:middle;" src="' + config.ico + '" alt="' + config.openTxt + '" /></a>').insertAfter(select);
		var input	= jQuery('<span id="text-span" style="color:#444444;margin-left:5px;"></span>').insertAfter(icon);
		var loc		= '';
		...
		if (config.inputBG) {
			input.change(function () {
				$('#colour-block').css('background-color','#' + input.val());
                $('#text-span').text('Current colour is #' + input.val());
			});
